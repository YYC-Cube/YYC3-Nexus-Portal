...
export function useFloatingPanel(options: UseFloatingPanelOptions = {}): UseFloatingPanelReturn {
  ...
  const closePanel = useCallback(() => { setIsOpen(false); setIsMaximized(false); }, []);
  ...
  return { isOpen, isMaximized, activeTab, openPanel, closePanel, togglePanel, toggleMaximize, setActiveTab, panelClass };
}
